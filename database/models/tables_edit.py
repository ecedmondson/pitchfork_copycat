...
class AlbumTable(DBConnection):
    ...
    def _touch_helper(self, search_keyword):
        ...
        select = select_statement("review")
        where_ = where("reivew", "album_id", 
        # LEFT OFF HERE
        # NEED TO DO ALL THE TOUCHES
        # SEARCH AGAINST ENTITY
        # FULL SEARCHES
        # AND PARSES
...
class SearchSQL(DBConnection):
    ...
    def _check_if_search_keyword_in_database(self, search_keyword, search_by, table, column):
        where_operator = "like"
    ...
    def execute_search(self, search_keyword, search_by):
        ...
        table = table()
        queries = self._check_if_search_keyword_in_database(search_keyword, search_by, table_name, column)
        # Option 2
        if not queries:
            return []
        # Option 3
        ...
        ### Below needs to be deleted. It is only there for reference.
        if search_by == "album":
            # This is temporary
