...
class AlbumTable(DBConnection):
    ...
    def _does_album_exist(self, album):
        statement = "SELECT id from album where album.title = %s;"
        print(f"DEBUG IF ALBUM EXISTs: {statement} and {artist}")
        logging.debug(f"DEBUG IF ALBUM EXISTs: {statement} and {artist}")
        return self.execute_query(statement, (artist,)).fetchall()

    def _validate_insert_album_data(self, data):
        if not data['albumName']:
        ...
        if self._does_album_exist(data['albumName']):
	...
	def create_new_album(self, album_data):
    	validated = self._validate_insert_album_data(album_data)
    	if isinstance(validated, str):
        	return validated
    	statement = (
    		"""
    		INSERT INTO album (title, artist_id, cover_art, release_date, publisher, spotify_url) 
    		values (%s, %s, %s, %s, %s, %s);
    		"""
  		)
  		id = self.execute_query(statement).lastrowid
